...
def abrir_posicao(client: Client, symbol: str, direcao: str, preco: float, banca: float, qualidade: str = "NORMAL", risco_base: float = None, score_entrada: int = 0) -> None:
    ...
    if direcao == "LONG" and n_long >= 50:
        log.info(f"  {symbol}: BLOQUEADO — ja tem {n_long} LONGs (max 18)")
        return
    if direcao == "SHORT" and n_short >= 50:
                    ...
                    if tempo_desde_ultimo < 600:
                        ...
                        try:
                            ...
                            if pode_3x:
                                ...
                                # O 3x usa o saldo DISPONIVEL. Se nao tem, NAO faz.
                                saldo_livre = get_banca(client)
                                margem_3x_necessaria = banca * 0.05  # 5% da banca (DCA dinamico)
                                if saldo_livre < margem_3x_necessaria:
                                    log.info(f"  {symbol}: Score {score} bom mas saldo livre ${saldo_livre:.2f} < ${margem_3x_necessaria:.2f} — sem evacuacao, aguarda")
                                    continue
